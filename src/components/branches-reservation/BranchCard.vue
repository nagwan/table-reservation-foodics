...
export default {
  ...
  computed: {
    getTablesCount() {
      ...
      this.branch.sections.forEach((section) => {
        count += section.tables.filter((table) => table.accepts_reservations)
          .length;
      });
  ...
  methods: {
    ...
    disableBranchReservations({ value }) {
      this.$emit('update-branch-reservation-settings', {
        id: this.branch.id,
        value,
      });
    },
    ...
    <div class="shadow-1 radius-8 p-20 branch-card">
        ...
        >
            ...
            <BaseToggle
                ...
                @update-value="disableBranchReservations"
            />
        </div>
    </div>
</template>
